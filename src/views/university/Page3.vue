        ...
        <el-row :gutter="20">
          <el-col :span="6">
            <el-form-item label="大学名称">
              <el-input
                v-model="searchForm.universityNameChinese"
                placeholder="请输入大学名称"
              ></el-input>
            </el-form-item>
          </el-col>
  ...
  setup() {
    ...
    const updateChartOption = () => {
      const filteredSeries = allSeries.value.filter((series) =>
        selectedLegends.value.includes(series.name)
      );
      chartOption.value = {
    ...
    const enableDrag = () => {
      const dialogHeaderEl = document.querySelector(
        ".el-dialog.draggable-dialog .el-dialog__header"
      );
